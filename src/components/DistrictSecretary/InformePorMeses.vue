  ...
  setup() {
    ...
    */
    const getAllTheMonthsThatHaveInformation = async (year: number) => {
      try {
        const responses = await handleRequest('put', `/getAllTheMonthsThatHaveInformationDos`, {}, year)
    ...
    /*
      Abrir el componente "Informe mes seleccionado". 
    ...
    */
    const open_informeMesX = (mes: Month) => {
      ...
      if(mes.haveInformation) {
        router.replace({ name: 'Informe mes seleccionado', params:{ month_id: mes.id} })
      } else {
